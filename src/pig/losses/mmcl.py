...
# non_matches_axis is different entities (e.g. other keypoints)
    ...
    def forward(self, x, status):
        ...
        # N x NM
        matches_loss = torch.amin(matches_dist_matrix,dim=(-1,-2))
        # print(f"matches_loss={matches_loss[0]}")
        # the mean of the loss
        matches_loss = torch.min(matches_loss)
        ...
        non_matches_loss= torch.amax(non_matches_dist_matrix,dim=(-1,-2))
        non_matches_loss=torch.max(non_matches_loss)
        # compute the contrastive loss
        contrastive_loss=torch.clamp(non_matches_loss-matches_loss+self.margin,min=0)
